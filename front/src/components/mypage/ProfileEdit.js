...
function ProfileEdit({ toggleEditForm, updateUser, user }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const ImageEdit = imageInfo && setImageForm({ ...form, profile_image: imageInfo });
        ...
        <Grid item xs={5}>
            <form onSubmit={handleSubmit}>
                ...
                <Stack direction="row" spacing={2} sx={{ mt: 2, justifyContent: 'center' }}>
                    <Button variant="contained" type="submit" disableElevation disableRipple>
                        {' '}
                        확인{' '}
                    </Button>
                    <Button type="reset" onClick={() => toggleEditForm()} variant="outlined">
                        {' '}
                        취소{' '}
                    </Button>
                </Stack>
